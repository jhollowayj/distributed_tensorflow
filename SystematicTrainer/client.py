...
class ModDNN_ZMQ_Client:
    ...
    def ZMQ_setup(self, config):
        ...
        if config['just_one_server']:
            print "Adding just one server @ '{}'".format(config['serverHostName'])
    ...
    def sendGradients(self, gradients, network_type = NetworkType.World):
        ...
        print self.network_id_lookup[network_type],
        ...
        print "...",
        res = server.grad_send.send(msg, flags=0, track=True)
    ...
    def poll_once(self):
        ...
        socks = dict( self.poller.poll( 300 ) ) # poll param == sleep time between polls
